...
aws ec2 describe-subnets --filters "Name=vpc-id,Values=$vpc_id" --query 'Subnets[*].{SubnetId: SubnetId,AvailabilityZone: AvailabilityZone,CidrBlock: CidrBlock}' --region $aws_region --output table | tail -n +6 | awk '{print $6}' | xargs -Isubnet_id aws efs create-mount-target --file-system-id $file_system_id --security-groups $security_group_id --region $aws_region --subnet-id subnet_id

# Create the EFS storage class
# echo "{\"apiVersion\": \"storage.k8s.io/v1\", \"kind\": \"StorageClass\", \"provisioner\": \"efs.csi.aws.com\", \"metadata\": {\"name\": \"efs-sc\", \"annotations\": {\"storageclass.kubernetes.io/is-default-class\": \"true\"}}, \"parameters\": {\"server\": \"${file_system_id}.efs.us-west-2.amazonaws.com\", \"path\": \"/\", \"readOnly\": \"false\"}}" | kubectl apply -f -
...
# kubectl create namespace notebook-workspace
...
sleep 10
echo "Wait for all pods to start up and then visit the following URL"
